...
local groupBottom = display.newGroup()
...
local bkg = display.newImage( "background11.png", centerX, centerY+100 )  

local bullet
local bulletEn
local bullets = display.newGroup()
local player = display.newCircle( 50 , 545 , 25 )
...
prx , pry = player:localToContent(0,0)
...
local function shot( event )
  ...
  myLine = display.newLine( prx,pry, prx , pry+20)
      myLine:setStrokeColor(0.1, 0.2, 0.3)
      myLine.strokeWidth = 20
      myLine.alpha=0.8
      --groupPl:insert(myLine)
  ...
  elseif player.isFocus then
    ...
    player:setLinearVelocity(0,0)
      ...
      myLine = display.newLine( prx, pry, event.x,event.y)
      myLine:setStrokeColor(0.1, 0.2, 0.3)
      myLine.strokeWidth = 20
      myLine.alpha=0.8
      --groupPl:insert(myLine)
...
normDeltaY = deltaY / math.sqrt(math.pow(deltaX,2) + math.pow(deltaY,2))
    ...
    bullet.x = player.x
    bullet.y = player.y-50
    --if(state == 0 )then bullet:setFillColor("black") end
    if(state == 1 )then bullet:setFillColor(gray) end
...
bt.x = display.contentCenterX
bt.y = display.contentCenterY + 200

local bt2 = widget.newButton(
    {
        width =  50,
        height = 30,
        id = "button2",
...
bt2.x = display.contentCenterX + 200
bt2.y = display.contentCenterY + 200
...
groupPl:insert(player)
groupBottom:insert(borderBottom)
groupBottom:insert(borderRight)
groupBottom:insert(borderTop1)
groupBottom:insert(borderLeft)
groupBottom:insert(middleBottom)

--groupBottom:insert(bkg)
--groupBottom:insert(player)
--groupBottom:insert(en)
--groupBottom:insert(en2)

camera:add(groupPl,1)
camera:add(en,2)
camera:add(en2,3)
camera:add(bullets,4)
camera:add(groupBottom,5)
camera:add(bkg,6)
...
player:addEventListener("tap" , tapa )
Runtime:addEventListener("touch",touched)
timer.performWithDelay(1 , update , -1 )
timer.performWithDelay( 2000 , position , -1)
